...
async function clickMenuByText(
  ...
  const posResult = await session.cdp.send<{ result: { value: string } }>(
    "Runtime.evaluate",
    {
      expression: `
      (function() {
...
async function copyImageToClipboard(imagePath: string): Promise<void> {
  ...
  const result = spawnSync(
    "npx",
    ["-y", "bun", copyScript, "image", imagePath],
    { stdio: "inherit" },
  );
  if (result.status !== 0)
    throw new Error(`Failed to copy image: ${imagePath}`);
}
...
async function copyHtmlFromBrowser(
  ...
  console.log(
    "[wechat] Copying with execCommand('copy') via CDP (userGesture)...",
  );
  await cdp.send<{ result: { value: unknown } }>(
    "Runtime.evaluate",
    {
...
function parseHtmlMeta(htmlPath: string): {
  title: string;
  author: string;
  summary: string;
} {
...
async function selectAndReplacePlaceholder(
  ...
  const result = await session.cdp.send<{ result: { value: boolean } }>(
    "Runtime.evaluate",
    {
      expression: `
      (function() {
...
export async function postArticle(options: ArticleOptions): Promise<void> {
  ...
  if (effectiveTitle && effectiveTitle.length > 64)
    throw new Error(`Title too long: ${effectiveTitle.length} chars (max 64)`);
  if (!content && !effectiveHtmlFile)
  ...
  try {
    ...
    const editorTargetId = await waitForNewTab(
      cdp,
      initialIds,
      "mp.weixin.qq.com",
    );
    ...
    const { sessionId } = await cdp.send<{ sessionId: string }>(
      "Target.attachToTarget",
      { targetId: editorTargetId, flatten: true },
    );
    ...
    if (effectiveAuthor) {
      ...
      await evaluate(
        session,
        `document.querySelector('#author').value = ${JSON.stringify(effectiveAuthor)}; document.querySelector('#author').dispatchEvent(new Event('input', { bubbles: true }));`,
      );
    }
    ...
    if (effectiveHtmlFile && fs.existsSync(effectiveHtmlFile)) {
      ...
      if (contentImages.length > 0) {
        ...
        for (let i = 0; i < contentImages.length; i++) {
          ...
          console.log(
            `[wechat] [${i + 1}/${contentImages.length}] Processing: ${img.placeholder}`,
          );

          const found = await selectAndReplacePlaceholder(
            session,
            img.placeholder,
          );
          if (!found) {
          ...
          console.log(
            `[wechat] Copying image: ${path.basename(img.localPath)}`,
          );
    ...
    await evaluate(
      session,
      `document.querySelector('#js_submit button').click()`,
    );
    await sleep(3000);

    const saved = await evaluate<boolean>(
      session,
