...
WANGJIE_CONFIG_FILE = Path(os.environ.get("WANGJIE_CONFIG_FILE", "/home/ubuntu/wangjie.txt"))
...
def load_wangjie_config(path: Path = WANGJIE_CONFIG_FILE) -> Dict[str, str]:
    """Load OpenAI-compatible config from wangjie.txt if present.

    Supported values:
    - OPENAI_API_KEY from embedded JSON block
    - base_url from TOML section
    - model from top-level setting
    """
    if not path.exists():
        return {}
    ...
    key_match = re.search(r'"OPENAI_API_KEY"\s*:\s*"([^"]+)"', text)
    if key_match:
        out["OPENAI_API_KEY"] = key_match.group(1).strip()

    base_match = re.search(r'base_url\s*=\s*"([^"]+)"', text)
    if base_match:
        out["OPENAI_BASE_URL"] = base_match.group(1).strip()

    model_match = re.search(r'^\s*model\s*=\s*"([^"]+)"', text, re.MULTILINE)
    if model_match:
        out["OPENAI_MODEL_PIN"] = model_match.group(1).strip()
        out["OPENAI_MODEL_POLICY"] = "pinned"
...
def get_config() -> Dict[str, Any]:
    ...
    wangjie_env = load_wangjie_config()
    ...
    # OpenAI-compatible custom endpoint support (e.g. from wangjie.txt)
    config['OPENAI_BASE_URL'] = (
        os.environ.get('OPENAI_BASE_URL')
        or merged_env.get('OPENAI_BASE_URL')
        or wangjie_env.get('OPENAI_BASE_URL')
        or 'https://api.openai.com'
    )
    os.environ['OPENAI_BASE_URL'] = config['OPENAI_BASE_URL']
    config['PREFER_OPENAI_REDDIT'] = bool(
        os.environ.get('PREFER_OPENAI_REDDIT')
        or wangjie_env.get('OPENAI_API_KEY')
    )

    # Allow wangjie model pinning only when not explicitly set elsewhere
    if not config.get('OPENAI_MODEL_PIN') and wangjie_env.get('OPENAI_MODEL_PIN'):
        config['OPENAI_MODEL_PIN'] = wangjie_env.get('OPENAI_MODEL_PIN')
        config['OPENAI_MODEL_POLICY'] = 'pinned'
...
def get_reddit_source(config: Dict[str, Any]) -> Optional[str]:
    ...
    """
    if config.get('PREFER_OPENAI_REDDIT') and config.get('OPENAI_API_KEY') and config.get('OPENAI_AUTH_STATUS') == AUTH_STATUS_OK:
        return 'openai'
