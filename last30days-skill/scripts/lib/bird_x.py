...
def _subprocess_env() -> Dict[str, str]:
    ...
    # Ensure Node.js 22+ is used (nvm may not be active in subprocess PATH).
    # Bird uses `import ... with { type: 'json' }` which requires Node 20+.
    _nvm_node_dirs = []
    nvm_dir = os.environ.get("NVM_DIR", os.path.expanduser("~/.nvm"))
    nvm_versions = Path(nvm_dir) / "versions" / "node"
    if nvm_versions.exists():
        candidates = sorted(
            nvm_versions.iterdir(),
            key=lambda p: [int(x) for x in p.name.lstrip("v").split(".") if x.isdigit()],
            reverse=True,
        )
        for candidate in candidates:
            major = int(candidate.name.lstrip("v").split(".")[0]) if candidate.name.startswith("v") else 0
            if major >= 20:
                _nvm_node_dirs.append(str(candidate / "bin"))
                break
    if _nvm_node_dirs:
        env["PATH"] = ":".join(_nvm_node_dirs) + ":" + env.get("PATH", "")
    return env
