...
|---|---|---|
| `REDDIT_TOKEN_V2` | For Reddit | Reddit OAuth API (highest priority, no cost) |
| `OPENAI_API_KEY` | For Reddit | Reddit search via OpenAI responses API (fallback) |
...
### Reddit OAuth Token (REDDIT_TOKEN_V2)

The `token_v2` cookie from a logged-in Reddit session gives direct OAuth API access — free, no third-party API needed.

**How to get it:**
1. Log in to Reddit in your browser
2. Open DevTools → Application → Cookies → `https://www.reddit.com`
3. Copy the value of `token_v2`
4. Save to `~/.config/last30days/.env`: `REDDIT_TOKEN_V2=<value>`

**Notes:**
- Expires after ~24 hours; refresh by copying fresh cookie
- Rate limit: 60 requests/minute (well within normal use)
- Takes priority over Apify, ScrapeCreators, and OpenAI paths when set
