...
转录完成、逐字稿已保存后，**删除本次任务下载或生成的所有临时文件**：

- 下载的音频/视频文件（yt-dlp 输出的那个文件）
- Whisper 的中间 JSON（如果写到磁盘了）
- 任何其他本次任务产生的临时文件

**原则：你下载了什么，任务结束后就删什么。** 不依赖固定路径——用你在 Step 3/4 实际写入的路径来删除。
...
**转录完成后必须清理临时文件**
- 任务结束前，删除本次下载/生成的所有临时文件（音频、视频、中间 JSON）
- 用你实际写入时的路径来删除，不要写死任何目录
- 逐字稿是唯一需要保留的输出
